...
import iconDeleteItem from '../imgs/deleteItem.svg'
...
const DrawerCard: React.FC<DrawerCardProps> = ({id, code, title, img, price, count, producer, rating, overflowHidden, setLoading}) => {

    const dispatch = useDispatch()
    const cartItem = useSelector((state: any) => state.cart.cartItems.find((obj: {
        id: string, 
        code: string, 
        title: string,
        img: string,
        ...
        producer: string,
        rating: number
    }) => Number(obj.code) === Number(code)))
    
    let iconDelete = true 

    cartItem.count > 1 ? iconDelete = false : iconDelete = true
    ...
    const deleteItem = async (obj: {
        ...
        setLoading(true)
        try { 
            const { data } = await axios.get(`https://6241abd3042b562927a77458.mockapi.io/itemsOfCart?code=${obj.code}`)
            axios.delete('https://6241abd3042b562927a77458.mockapi.io/itemsOfCart/' + data[0].id) 
        }
        catch {alert('oi')}
        dispatch(removeItem(cartItem))
    }

    const minusItem = async (obj: {
        id: string, 
        code: string, 
        title: string,
        img: string,
